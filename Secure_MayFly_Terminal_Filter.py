...
# === Domestic and Terminal route filters ===
...
def parse_txt(file_content, filter_type, terminal_filter):
...
terminal_filter = st.radio("Select Terminal", ["All Terminals", "T5 Only", "T3 Only"])
filter_option = st.radio("Choose Load Filter", ["All Flights", "Flights above 90%", "Domestic"])

st.markdown("### Paste your MayFly data below")
text_input = st.text_area("Paste content from Ops Dashboard here")

if text_input:
    flights_df = parse_txt(text_input, filter_option, terminal_filter)

    if not flights_df.empty:
        st.success(f"Processed {len(flights_df)} flights ({filter_option}, {terminal_filter}).")
