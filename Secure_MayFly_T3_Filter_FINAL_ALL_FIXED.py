...
class BA_PDF(FPDF):
    ...
    def flight_table(self, data):
        headers = ['Flight No', 'Aircraft', 'Route',
                   'ETD', 'Conformance', 'Load']
    ...
    while i < len(lines):
        if lines[i].startswith("BA"):
            try:
                ...
                route = re.sub(r"\s+", "",
                               lines[i+3].strip().upper())
                std_match = re.search(
                    r"STD: \d{2} \w+ - (\d{2}:\d{2})z",
                    lines[i+4]
                )
                load_match = re.search(r"(\d{1,3})%Status",
                                       lines[i+8])
                if std_match and load_match:
                    etd_str = std_match.group(1)
                    load = int(load_match.group(1))
                    etd_utc = datetime.strptime(etd_str, "%H:%M")
                    etd_utc = utc_tz.localize(etd_utc)
                    etd_z1 = (etd_utc + timedelta(hours=1)
                              ).strftime("%H:%M")
                    conf = (etd_utc + timedelta(minutes=25)
                           ).strftime("%H:%M")
                    flights.append({
...
selected_date = st.date_input("Select MayFly Date",
                              datetime.today())
...
if text_input:
    ...
    if station == "T3":
        flights_df = flights_df[
            flights_df["Flight Number"].isin(T3_FLIGHTS)
        ]
    elif station == "T5":
        flights_df = flights_df[
            ~flights_df["Flight Number"].isin(T3_FLIGHTS)
    ...
    if not flights_df.empty:
        st.success(f"Processed {len(flights_df)} flights"
                   f" ({filter_option}, {station}).")
        pdf = BA_PDF(date_str, orientation='P',
                     unit='mm', format='A4')
        ...
        with open(pdf_output_path, "rb") as file:
            st.download_button(
                ...
                data=file,
                file_name=(
                    f"BA_MayFly_{date_str.replace(' ', '_')}.pdf"
                ),
        ...
        st.error(
            "No valid flights found with current filter."
        )
